...
- **評価機能:** 購入者は出品者に対して5段階の評価とコメントが行えます。
...
- 本番環境ではメールの設定が行われていないため、メール通知機能はご利用いただけません。ご使用の環境でテストや開発を行う際には、local環境でご確認ください。
- Googleログインに関しても、ドメインを取得していないため、本番環境ではご利用いただけません。そのため、本番環境では通常のユーザー登録およびログイン方法をご利用ください。
